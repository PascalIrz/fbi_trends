...
latin_recoding <- function(df, var_latin_name)
{
  ...
  df <- df %>% 
    mutate(!!var_latin_name := case_when(
      str_detect(!!var_latin_name, pattern = "Barbatula") ~ "Barbatula spp",
      str_detect(!!var_latin_name, pattern = "Carassius") ~ "Carassius spp",
      str_detect(!!var_latin_name, pattern = "Cottus") ~ "Cottus spp",
      str_detect(!!var_latin_name, pattern = "Esox") ~ "Esox spp",
      str_detect(!!var_latin_name, pattern = "Gobio") ~ "Gobio spp",
      str_detect(!!var_latin_name, pattern = "Phoxinus") ~ "Phoxinus spp",
      str_detect(!!var_latin_name, pattern = "Thymallus") ~ "Thymallus spp",
  ...
  
{
  ...
  if(smooth) g <- g + geom_smooth(method = lm, se = FALSE)
  
  g +
    labs(y = "Metric value", x = "Year") +
    facet_wrap(vars(!!var_facet), scales = scales) +
    scale_linetype_manual(values = c("dashed", "solid"), name = "", labels = NULL) +
   # theme(legend.position = "none") +
    scale_color_manual(
...
############################ g_covar_sp_indicator
# corrplot of species indocators across years (1 plot per sepcies)
#' Title
#'
#' @param df Dataframe with the data. Must have columns "esp_nom_latin" as well as the 
#'     population indicators (minimum "density" and "unexpected_absence")
#' @param species Character vector withe the specie latin names
#'
#' @return A list of the corrplots

g_covar_sp_indicator <- function(df, species) {
  
  g_covar_sp_indicator <- function(df, my_species) {
    df %>%
      filter(esp_nom_latin == my_species) %>%
      select(density:unexpected_absence) %>%
      cor() %>%
      corrplot::corrplot(title = my_species,
                         method = 'ellipse',
                         type = 'upper')
